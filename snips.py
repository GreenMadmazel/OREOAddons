...
from pyOreo.dB.snips_db import add_snip, get_snips, list_snip, rem_snip
from pyOreo.fns.tools import create_tl_btn, format_btn, get_msg_button

from . import events, get_string, mediainfo, udB, oreo_bot, oreo_cmd
from ._inline import something


@oreo_cmd(pattern="addsnip( (.*)|$)")
async def an(e):
    ...
    oreo_bot.add_handler(add_snips, events.NewMessage())


@oreo_cmd(pattern="remsnip( (.*)|$)")
async def rs(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    if not wrd:
    ...
    for z in xx:
        if k := get_snips(z):
            ...
            if rep:
                ...
                await oreo_bot.send_message(e.chat_id, msg, file=media)
